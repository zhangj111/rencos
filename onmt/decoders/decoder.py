...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        self.W_r = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
        # self.W_s = nn.Linear(self.hidden_size, self.hidden_size, bias=False)
        # self.W_x = nn.Linear(self.embeddings.word_vec_size, self.hidden_size, bias=True)
    ...
    def forward(self, tgt, memory_bank, memory_lengths=None,
        ...
        dec_state, dec_outs, attns = self._run_forward_pass(
            tgt, memory_bank, memory_lengths=memory_lengths, ref=ref)
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None, ref=None):
        """
        ...
        for _, emb_t in enumerate(emb.split(1)):
            ...
            #     decoder_output_ref, p_attn_ref = self.attn(
            #         rnn_output,
            #         ref_bank.transpose(0, 1),
            #         memory_lengths=ref_lengths)
            #     # decoder_output_ref = torch.mean(ref_bank, 0)
            #     # p_ref = torch.sigmoid(self.W_r(decoder_output_ref)+self.W_s(decoder_output)+self.W_x(emb_t))
            #     # decoder_output = self.W_r(torch.cat([decoder_output,decoder_output_ref], 1))
            #     decoder_output = 0.1*decoder_output + 0.9*decoder_output_ref
                # decoder_output /= 2.0
